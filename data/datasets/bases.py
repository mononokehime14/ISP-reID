...
class BaseDataset(object):
    ...
    def get_val_imagedata_info(self, data):
        pids = []
        for _, pid in data:
            pids += [pid]
        pids = set(pids)
...
class BaseValImageDataset(BaseDataset):
    """
    Base class of image reid dataset for validation only
    """

    def print_dataset_statistics(self, query, gallery):
        num_query_pids, num_query_imgs = self.get_val_imagedata_info(query)
        num_gallery_pids, num_gallery_imgs = self.get_val_imagedata_info(gallery)
